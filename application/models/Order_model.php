...
class Order_model extends CI_Model
{
    ...
    public function update_order($set, $where, $isjson = false, $table = 'order_items', $fromuser = false, $is_escape_array = true, $is_digital_product = 0)
    {
        ...
        if ($isjson == true) {
            ...
            if (count($res) >= 1) {
                ...
                foreach ($res  as $row) {
                    ...
                    if ($current_status == 'delivered') {
                        ...
                        $total = 0;
                        $final_total = 0;
                        // if ($table == "consignments" && $order_final_total[0]['is_cod_collected'] == 0) {
                        if ($table == "consignments") {
                            if (!empty($order)) {
                                ...
                                // update_details(['is_cod_collected' => 1], ['id' => $order[0]['order_id']], 'orders');
                            }
                        }
                    }
                    ++$i;
            ...
            //     $this->db->set($set)->where_in('id', $where)->where(['delivery_boy_id' => 74])->update($table);
            // } else {
    ...
    public function place_order($data)
    {
        ...
        $total = 0;
        $product_variant = $this->db->select('pv.*,GROUP_CONCAT(tax.percentage) as tax_percentage ,GROUP_CONCAT(tax.id) as tax_ids,GROUP_CONCAT(tax.title) as tax_name,p.seller_id,p.name as product_name,p.type as product_type,p.is_prices_inclusive_tax,p.is_attachment_required,p.download_link')
    ...
    public function consignment_view($order_id = NUll, $seller_id = NULL, $delivery_boy_id = NULL)
    {
        ...
        $search_res = $this->db->select('DISTINCT(c.id) as id, c.order_id as order_id, c.name as name, c.active_status as status, c.created_at, c.otp as otp')
            ->join('consignment_items ci', 'ci.consignment_id = c.id')
            ->join('orders o', 'c.order_id = o.id')
            ->join('order_items oi', 'oi.id = ci.order_item_id')
            ->join('users u', 'u.id = o.user_id', 'left');
        ...
        foreach ($consignment_list as $row) {
            ...
            $item_detail = $this->db->select("oi.*,u.username,c.active_status as active_status,c.delivery_boy_id, ci.*,sd.store_name,o.payment_method,o.mobile, p.image")
            ...
            $order_link = "<a href='" . base_url((($this->ion_auth->is_seller()) ? "seller" : "admin") . "/orders/edit_orders?edit_id=" . $row['order_id']) . "' target='_blank'>" . $row['order_id'] . "</a>";
            $product_name = [];
            $qtys = [];
            ...
            $tempRow['payment_method'] = $item_detail[0]['payment_method'];
            ...
            if ($this->ion_auth->is_seller()) {
                $operate .= '<button type="button" class="btn btn-success mx-1 btn-xs consignment_status_btn" data-id="' . $row['id'] . '" data-consignment-name="' . $row['name'] . '" data-status="' . $row['status'] . '" data-items=\'' . json_encode($item_detail) . '\' data-toggle="modal" data-target="#consignment_status_modal"><i class="fa fa-pen"></i></button>';
