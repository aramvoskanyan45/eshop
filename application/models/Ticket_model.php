...
class Ticket_model extends CI_Model
{
    ...
    function get_user_ticket_list($user_id)
    {
        $offset = 0;
        $limit = 10;
        $sort = 't.id';
        ...
        $count_res = $this->db->select(' COUNT(u.id) as `total`')->join('ticket_types tty', 'tty.id=t.ticket_type_id', 'left')->join('users u', 'u.id=t.user_id', 'left')->where('t.user_id', $user_id);
        ...
        $search_res = $this->db->select('t.*,tty.title,u.username')->join('ticket_types tty', 'tty.id=t.ticket_type_id', 'left')->join('users u', 'u.id=t.user_id', 'left')->where('t.user_id', $user_id);
        ...
        foreach ($cat_search_res as $row) {
            ...
            $operate = '<a href="javascript:void(0)" class="view_ticket btn btn-success action-btn btn-xs" data-id=' . $row['id'] . ' data-username=' . $row['username'] . ' data-date_created=' . $row['date_created'] . ' data-subject=' . $row['subject'] . ' data-status=' . $row['status'] . ' data-ticket_type="' . $row['title'] . '" title="View" data-bs-toggle="modal" data-bs-target="#address-modal" ><i class="uil uil-edit"></i></a>';
